...
export default function CandidateDetail() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 py-24 text-sm text-ink-faint dark:text-night-ink-faint">
  ...
  if (error) {
    return (
      <div className="px-8 py-16 text-center">
        <p className="text-sm font-medium text-ink dark:text-night-ink">{error}</p>
        <p className="mt-1 text-sm text-ink-faint dark:text-night-ink-faint">
          Check your session and try again, or return to the list.
        </p>
        <Link to="/onboardings" className="mt-3 inline-block text-sm font-medium text-brand-dark dark:text-brand-night hover:underline">
          Back to onboardings
        </Link>
      </div>
    )
  ...
  return (
    <div className="pb-16">
      <div className="border-b border-border-soft dark:border-night-line-soft px-8 py-6">
        <button
          onClick={() => navigate('/onboardings')}
          className="mb-4 flex items-center gap-1.5 text-[13px] font-medium text-ink-soft dark:text-night-ink-soft hover:text-ink dark:hover:text-night-ink"
        >
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="font-display text-2xl font-semibold tracking-tight text-ink dark:text-night-ink">{candidate.full_name}</h1>
            ...
            <p className="mt-0.5 text-sm text-ink-faint dark:text-night-ink-faint">{candidate.position}</p>
      ...
      <div className="grid grid-cols-1 gap-6 px-8 py-7 lg:grid-cols-[1.6fr_1fr]">
        <Card className="p-6">
          <h2 className="font-display text-base font-semibold text-ink dark:text-night-ink">Required documents</h2>
        ...
        <Card className="p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="font-display text-base font-semibold text-ink dark:text-night-ink">Reminder activity</h2>
          ...
          <div className="mt-5">
            {reminders.length === 0 ? (
              <p className="text-sm text-ink-faint dark:text-night-ink-faint">No reminders sent yet.</p>
            ) : (
              <ChaseTrail
      ...
      >
        {previewDoc && (
          <div>
            {previewBusy && (
              <div className="flex h-48 items-center justify-center gap-2 rounded-xl border border-dashed border-border dark:border-night-line bg-surface-sunken/50 dark:bg-night-sunken text-sm text-ink-faint dark:text-night-ink-faint">
                <Loader2 className="h-4 w-4 animate-spin" /> Requesting secure access URL…
              </div>
            )}
            {!previewBusy && previewUrl && previewMime === 'application/pdf' && (
              <iframe src={previewUrl} title={previewDoc.name} className="h-[60vh] w-full rounded-xl border border-border dark:border-night-line" />
            )}
            {!previewBusy && previewUrl && previewMime !== 'application/pdf' && (
              <div className="flex justify-center rounded-xl border border-border dark:border-night-line bg-surface-sunken/40 dark:bg-night-sunken p-3">
                <img src={previewUrl} alt={previewDoc.name} className="max-h-[60vh] rounded-lg object-contain" />
              </div>
            )}
            {!previewBusy && !previewUrl && (
              <div className="flex h-40 items-center justify-center rounded-xl border border-dashed border-border dark:border-night-line bg-surface-sunken/50 dark:bg-night-sunken">
                <div className="flex flex-col items-center gap-2 text-ink-faint dark:text-night-ink-faint">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2.5 text-[13px]">
              <dt className="text-ink-faint dark:text-night-ink-faint">File</dt>
              <dd className="text-right text-ink-soft dark:text-night-ink-soft">{previewDoc.file_name}</dd>
              <dt className="text-ink-faint dark:text-night-ink-faint">Uploaded</dt>
              <dd className="text-right text-ink-soft dark:text-night-ink-soft">{formatDate(previewDoc.uploaded_at)}</dd>
              <dt className="text-ink-faint dark:text-night-ink-faint">Verification</dt>
              <dd className="text-right">
                <Badge status={previewDoc.verification_status} withIcon={false} />
              </dd>
            </dl>
            <p className="mt-3 text-xs text-ink-faint dark:text-night-ink-faint">
